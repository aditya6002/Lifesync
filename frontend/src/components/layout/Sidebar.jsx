  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderBottom: `1px solid ${C.glassBorder}`, marginBottom: 12,
        ...
        <div style={{
          ...
          fontSize: 15, boxShadow: "0 4px 12px rgba(124,58,237,.3)",
        }}>✦</div>
        {open && (
          <span style={{ fontFamily: FONTS.display, fontSize: 17, color: C.text, fontWeight: 700 }}>
            Lumina
          </span>
        )}
      </div>
      ...
      {NAV_ITEMS.map(item => (
        <button
          ...
          style={{
            ...
            justifyContent: open ? "flex-start" : "center",
            background: active === item.id ? "rgba(124,58,237,.15)" : "transparent",
            border: "none",
            borderLeft: active === item.id ? `3px solid ${C.violet}` : "3px solid transparent",
            color: active === item.id ? "#c4b5fd" : C.textDim,
            cursor: "pointer", transition: "all .15s",
            width: "100%", marginBottom: 2, fontSize: 13, fontWeight: 500,
          }}
        >
      ...
      {open && (
        <div style={{
          ...
          <button
            onClick={onLogout}
            style={{
              width: "100%", marginTop: 8, padding: "5px", borderRadius: 7,
              background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.2)",
              color: C.red, cursor: "pointer", fontSize: 11, fontWeight: 500,
            }}
          >Sign Out</button>
        </div>
      )}
      ...
      <button
        onClick={() => setOpen(o => !o)}
        style={{
          margin: open ? "0 12px" : "0 8px", padding: "8px", borderRadius: 8,
