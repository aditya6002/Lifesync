...
export default function NotesPage({ toast }) {
  ...
  const save = () => {
    ...
    if (modal.t === "add") {
      setNotes((ns) => [{ ...form, id: uid(), updated: "Just now" }, ...ns]);
      toast("Note saved ✓");
    } else {
      setNotes((ns) =>
        ns.map((n) =>
          n.id === modal.d.id ? { ...form, id: n.id, updated: "Just now" } : n,
        ),
      );
      toast("Updated ✓");
    }
  ...
  const filtered =
    folder === "All" ? notes : notes.filter((n) => n.tag === folder);
  ...
  return (
    <div style={{ display: "flex", gap: 16 }}>
        ...
        {["All", ...NOTE_TAGS].map((fdr) => (
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              fontSize: 12,
              color: folder === fdr ? "#c4b5fd" : C.textDim,
              background:
                folder === fdr ? "rgba(124,58,237,.15)" : "transparent",
              border: "none",
              marginBottom: 2,
      ...
      >
        ...
        >
          <h2
            style={{ fontFamily: FONTS.display, fontSize: 22, color: C.text }}
          >
            Notes{" "}
            {folder !== "All" && (
              <span style={{ fontSize: 14, color: C.textDim }}>/ {folder}</span>
            )}
          </h2>
          ...
          {filtered.map((n) => (
            ...
            >
              ...
              <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
                <Btn variant="ai" small onClick={(e) => e.stopPropagation()}>
                  ⟡ Summarize
                </Btn>
                <button
                  onClick={(e) => {
      ...
      {(modal?.t === "add" || modal?.t === "edit") && (
        <Modal
          title={modal.t === "add" ? "New Note" : "Edit Note"}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  {NOTE_COLS.map((c) => (
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: 7,
                        background: c,
                        border: form.color === c ? "3px solid #fff" : "none",
                        cursor: "pointer",
                      }}
                    />
                  ))}
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <Btn onClick={save} disabled={!form.title}>
                {modal.t === "add" ? "Save Note" : "Update Note"}
              </Btn>
      ...
      {modal?.t === "view" && (
        ...
        >
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
            <Badge label={modal.d.tag} color={modal.d.color} />
            <span style={{ fontSize: 11, color: C.textDim }}>
              Updated {modal.d.updated}
            </span>
          </div>
          <div
            style={{
