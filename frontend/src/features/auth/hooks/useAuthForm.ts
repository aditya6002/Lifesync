...
import type { LoginPayload, SignupPayload } from "../../../shared/types";
...
export function useSignupForm() {
  ...
  const [loading, setLoading]= useState(false);
  const [error,   setError]  = useState("");
  const [interests, setInterests] = useState<string[]>(["📚 Student", "💰 Budget tracking"]);
  ...
  const submit = async () => {
    setLoading(true);
    try {
      await signup({ name, email, password: pass } as SignupPayload);
    } catch (e: unknown) {
  ...
  return {
    step, setStep, name, setName, email, setEmail,
    pass, setPass, confirm, setConfirm,
    interests, toggleInterest,
    loading, error, setError, nextStep, submit,
  };
}
