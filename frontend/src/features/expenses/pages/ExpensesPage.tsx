...
import { Glass, Btn, Badge, FInput, FSelect } from "../../../shared/components/ui/Atoms";
...
const CAT_CFG: Record<string, { icon:string; color:string }> = {
  Food:{ icon:"🍜", color:"#f97316" }, Travel:{ icon:"🚇", color:"#3b82f6" },
  Study:{ icon:"📚", color:"#a855f7" }, Health:{ icon:"🏋️", color:"#ec4899" },
  Entertainment:{ icon:"🎬", color:"#6366f1" }, Shopping:{ icon:"🛍️", color:"#06b6d4" },
  Income:{ icon:"💰", color:"#22c55e" }, Other:{ icon:"📦", color:"#475569" },
};
...
export default function ExpensesPage() {
  ...
  const { modal, form, isValid, openAdd, openEdit, openView, closeModal, setField, buildAmount } = useExpenseForm();
  ...
  return (
    <div className="screen-in" style={{ display:"flex", flexDirection:"column", gap:18 }}>
      ...
      <Glass style={{ padding:16, background:"rgba(124,58,237,.1)", border:"1px solid rgba(124,58,237,.25)" }}>
        <div style={{ display:"flex", gap:10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:12, color:C.textMid, marginTop:3 }}>You've spent ₹{total.toLocaleString()} this month. Meal-prepping 3×/week could save ~₹400.</div>
          ...
          {filtered.map(e => (
            <div key={e.id} className="hov-card" onClick={() => openView(e)} style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px", borderRadius:12, background:"rgba(255,255,255,.03)", border:`1px solid ${C.glassBorder}`, cursor:"pointer", transition:"all .2s" }}>
              ...
              <div style={{ display:"flex", gap:4 }}>
                ...
                <button onClick={ev => { ev.stopPropagation(); handleDelete(e.id); }} style={{ padding:"4px 8px", borderRadius:7, background:"rgba(239,68,68,.08)", border:"none", color:C.red, cursor:"pointer", fontSize:12 }}>🗑</button>
              </div>
            </div>
          ))}
      ...
      {(modal.type === "add" || modal.type === "edit") && (
        <Modal title={modal.type === "add" ? "Add Expense" : "Edit Expense"} onClose={closeModal}>
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div style={{ display:"flex", gap:8 }}>
              {(["expense","income"] as const).map(tp => (
                <button key={tp} onClick={() => setField("type", tp)} style={{ flex:1, padding:"8px", borderRadius:10, border:form.type===tp?"none":`1px solid ${C.glassBorder}`, background:form.type===tp?(tp==="income"?C.green:C.violet):"rgba(255,255,255,.04)", color:"#fff", cursor:"pointer", fontWeight:600, fontSize:13, textTransform:"capitalize" }}>{tp}</button>
              ))}
            </div>
            <FInput label="Name" value={form.name} onChange={v => setField("name",v)} placeholder="e.g. Zomato Order" required />
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              <FInput label="Amount (₹)" type="number" value={String(form.amount)} onChange={v => setField("amount", Number(v))} placeholder="0" required />
              <FSelect label="Category" value={form.cat} onChange={v => setField("cat", v as ExpenseCategory)}
                options={Object.keys(CAT_CFG).filter(k => k !== "Income")} />
            </div>
            <FInput label="Date" type="date" value={form.date} onChange={v => setField("date",v)} />
            <FInput label="Note (optional)" value={form.note ?? ""} onChange={v => setField("note",v)} placeholder="Any note..." />
            <div style={{ display:"flex", gap:10 }}>
              <Btn onClick={handleSave} disabled={!isValid}>{modal.type === "add" ? "Add Expense" : "Save Changes"}</Btn>
      ...
      {modal.type === "view" && modal.data && (
        <ViewModal title={modal.data.name} onClose={closeModal} onEdit={() => openEdit(modal.data!)} onDelete={() => handleDelete(modal.data!.id)}>
          <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:16 }}>
            <div style={{ width:52, height:52, borderRadius:14, background:(CAT_CFG[modal.data.cat]?.color??C.textDim)+"22", display:"flex", alignItems:"center", justifyContent:"center", fontSize:28 }}>
              {modal.data.icon ?? CAT_CFG[modal.data.cat]?.icon}
            </div>
            <div>
              <div style={{ fontSize:26, fontWeight:700, color:modal.data.amount>0?C.green:C.text, fontFamily:FONTS.display }}>
                {modal.data.amount>0?"+":""}₹{Math.abs(modal.data.amount).toLocaleString()}
              </div>
              <div style={{ fontSize:12, color:C.textDim }}>{modal.data.cat} · {fmtDate(modal.data.date)}</div>
            </div>
          </div>
          {modal.data.note && <div style={{ padding:14, background:"rgba(255,255,255,.03)", borderRadius:10, fontSize:13, color:C.textMid }}>{modal.data.note}</div>}
        </ViewModal>
      )}
    </div>
  );
