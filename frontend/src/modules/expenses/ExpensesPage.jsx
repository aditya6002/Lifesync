...
export default function ExpensesPage({ toast }) {
  ...
  const save = () => {
    ...
    if (modal.t === "add") {
      setExpenses((es) => [{ ...form, id: uid(), amount: amt, icon }, ...es]);
      toast("Expense added ✓");
    } else {
      setExpenses((es) =>
        es.map((x) =>
          x.id === modal.d.id ? { ...form, id: x.id, amount: amt, icon } : x,
        ),
      );
      toast("Updated ✓");
    }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: C.textMid, marginTop: 3 }}>
              You've spent ₹{total.toLocaleString()} this month. Food is your
              top category. Meal-prepping 3×/week could save ~₹400.
            </div>
            <div
              style={{
                display: "flex",
      ...
      {(modal?.t === "add" || modal?.t === "edit") && (
        <Modal
          title={modal.t === "add" ? "Add Expense" : "Edit Expense"}
        ...
        >
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            />
            <div style={{ display: "flex", gap: 10 }}>
              <Btn onClick={save} disabled={!form.name || !form.amount}>
                {modal.t === "add" ? "Add Expense" : "Save Changes"}
              </Btn>
          ...
          >
            <div
              style={{
                ...
                height: 52,
                borderRadius: 14,
                background: (CAT_CFG[modal.d.cat]?.color || C.textDim) + "22",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                fontSize: 28,
            ...
            <div>
              <div
                style={{
                  fontSize: 26,
                  fontWeight: 700,
                  color: modal.d.amount > 0 ? C.green : C.text,
                  fontFamily: FONTS.display,
                }}
              >
                {modal.d.amount > 0 ? "+" : ""}₹
                {Math.abs(modal.d.amount).toLocaleString()}
              </div>
              <div style={{ fontSize: 12, color: C.textDim }}>
                {modal.d.cat} · {fmtDate(modal.d.date)}
              </div>
            </div>
          </div>
          {modal.d.note && (
